statystyka aby wyznaczyc survival(dose_rate) = intercept + dose_rate*coefficient
linear regression?

#coefficients for linear functions survival(dose_rate) = intercept + dose_rate*coefficient
> coef(lm_bs)
(Intercept)  dose_rates 
  0.4733143   0.1574475 
> coef(lm_bs_pol)
(Intercept)  dose_rates 
  1.1355350   0.4009246 
...
dose_rates <- c(19.4, 22.2, 22.6, 22.8, 23.1, 23.8, 24.6, 24.9, 24.6, 23.9, 21.3, 18.3, 15.3, 12.7, 10.5, 8.7, 5.9, 4.0, 1.8, 0.8, 0.3, 0.13, 0.06)
...
radiation_susceptibility_data_drops <- filter(radiation_susceptibility_data, dose_rates != 19.40 & dose_rates != 22.2 & dose_rates != 22.6 & dose_rates != 22.8 & dose_rates != 23.1 & dose_rates != 23.8)

radiation_susceptibility_data_longer <- pivot_longer(radiation_susceptibility_data_drops,
                        cols = -dose_rates,          
                        names_to = "group", 
                        values_to = "kill_frequency")  


ggplot(radiation_susceptibility_data_longer, aes(x = dose_rates, y = kill_frequency, color = group)) + geom_point() + geom_line()
...
ggplot(radiation_susceptibility_data_drops, aes(x = dose_rates, y = kill_freq_per_year_hs)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ log(x), se = FALSE)

ggplot(radiation_susceptibility_data_longer, aes(x = dose_rates, y = kill_frequency, color = group)) + geom_point() + geom_smooth(method = "lm", formula = y ~ log(x), se = FALSE)
